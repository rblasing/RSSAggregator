...
namespace Util
{
   ...
   /// go
   /// CREATE FUNCTION UdfSplitString(@s NVARCHAR(MAX), @excludeArticles BIT, @excludeConjunctions BIT, @excludePrepositions BIT, @excludePronouns BIT, @excludeCommon BIT) 
   ...
   public static class SQLServerStringUtils
   {
      ...
      readonly static string[] CommonIgnorable =
      {
         "are", "back", "be", "been", "big", "can", "could", "day", "did", "do",
         "does", "doesn't", "ever", "every", "get", "gets", "got", "had", "has",
         "have", "here", "how", "know", "i'm", "if", "is", "isn't", "let", "live",
         "make", "man", "may", "might", "more", "much", "must", "new", "news",
         "next", "no", "not", "now", "only", "put", "said", "say", "says", "seen",
         "set", "should", "still", "then", "too", "top", "until", "was", "were",
         "when", "while", "why", "will", "would",
      };
         ...
         bool distinctOnly)
      {
         ...
         if (excludeCommon)
            words = words.Where(w => !CommonIgnorable.Contains(w.ToLower())).ToArray();
         ...
         SqlBoolean excludeCommon)
      {
         ...
         return SplitString(s.Value, excludeArticles.Value,
            ...
            excludePronouns.Value, excludeCommon.Value, false);
      }
