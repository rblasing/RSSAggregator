...
namespace RSSWCFSvc
{
   ...
   public class RSSService : blasingame.RSS.xsd.RSS
   {
      ...
      public getItemsResponse getItemsByKeyword(getItemsByKeywordRequest request)
      {
         ...
         getItemsResponse rsp = new getItemsResponse();
         rsp.Body = new getItemsResponseBody();
         ...
         try
         {
            ...
            ItemRow[] items = dal.getItemsByKeywords(request.Body.keyword);
      ...
      public getItemsResponse getItemsByRange(getItemsByRangeRequest request)
      {
         ...
         try
         {
            ...
            ItemRow[] items = dal.getItemsByRange(request.Body.minDateTime,
      ...
      public getItemsResponse getTopItems(getTopItemsRequest request)
      {
         ...
         try
         {
            ...
            DAL dal = new DAL(dbConn);
            RSS.ItemRow[] items = dal.getTopItems((int)request.Body.itemCount);
            rsp.Body.items = BuildItemList(items);
         }
         catch (SqlException se)
         {
      ...
      private static ItemList BuildItemList(RSS.ItemRow[] items)
      {
         ...
         foreach (RSS.ItemRow item in items)
         {
            blasingame.RSS.xsd.Item i = new blasingame.RSS.xsd.Item();
            i.description = item.description;
            i.link = new Uri(item.url);
            i.pubDate = item.pubDate;
            i.publisher = item.feedName;
            i.title = item.title;
